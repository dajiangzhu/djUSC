...
public class ESL_ExploreTemplate_CalculateVR {
	...
	public void initialization() {
		...
		for (int s = subStartID; s <= subEndID; s++) {
			String configFile = ESL_OutPutDir+"/"+subStartID+"_"+subEndID+"/"+s+"/config_sub_"+s+".txt";
			...
			String curFile = ESL_OutPutDir+"/"+subStartID+"_"+subEndID+"/"+s+"/OptDicIndex_"+lastLineArray[0].trim()+"/sub_"+s+"_OptDicIndex_"+lastLineArray[0].trim()+"_Round_"+lastLineArray[1].trim()+"_D.txt";
								...
								// template
		{
			...
			for(int i=0;i<optDicIndex;i++)
			{
				double[][] tmpSigTemplate = this.trsposeM(DicccolUtilIO
						.loadFileAsArray(
								ESL_OutPutDir+"/"+subStartID+"_"+subEndID+"/TemplateSig_"
	...
	public void findTheTemplate() {
		...
		DicccolUtilIO.writeArrayListToFile(VROutPut,
				ESL_OutPutDir+"/"+subStartID+"_"+subEndID+"/"+currentSubID+"/OptDicIndex_"+optDicIndex+"/VR_sub_" + currentSubID
